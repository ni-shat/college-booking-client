...
const SignUp = () => {
    ...
    const onSubmit = data => {
        ...
        createUser(data.email, data.password)
            .then(result => {
                ...
                updateUserProfile(data.name, "") // update user profile
                    .then(() => {
                        const saveUser = {
                            name: data.name,
                            email: data.email
    ...
    return (
        <div className='bg-white h-screen bg-cover bg-center'>
            <Helmet>
                        ...
                        <div className="card w-full pb-10  flex-shrink-0 shadow-2xl bg-base-100  ">
                            <form onSubmit={handleSubmit(onSubmit)} className="card-body">
                                ...
                                <div className="form-control mt-6 relative">
                                    <input className="btn bg-lime-700 text-white" type="submit" value={isLoading ? "Loading..." : "Sign up"} />
                                    {
                                        isLoading && <BeatLoader className='absolute top-2/4 -translate-y-2.5 right-20 ' color="#82faac" />
                                    }
                                </div>
                            </form>
                            <div className='card-body -mt-10 -mb-5 flex flex-col items-center'>
                                ...
                                <div className='flex mt-5 justify-center'>
                                    <Link to='/login'>Already Have an account? <span className='text-lime-700 font-bold'>log in</span></Link>
                                </div>
                            </div>
